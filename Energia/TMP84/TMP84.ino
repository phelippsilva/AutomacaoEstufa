/*
  AnalogReadSerial
  Reads an analog input on pin A3, prints the result to the serial monitor.
  Attach the center pin of a potentiometer to pin A3, and the outside pins to ~3V and ground.
  
  Hardware Required:
  * MSP-EXP430G2 LaunchPad
  * 10-kilohm Potentiometer
  * hook-up wire

  This example code is in the public domain.
*/

// the setup routine runs once when you press reset:
void setup() {
  // initialize serial communication at 9600 bits per second:
  Serial.begin(9600); // msp430g2231 must use 4800
}

// the loop routine runs over and over again forever:
void loop() {
  // read the input on analog pin A3:
  int sensorValue = analogRead(A0);
  float convertido = 0.0;
      Serial.print("Antes de converter: ");
    Serial.print(sensorValue);
      convertido = map(sensorValue, 0, 1023, 0.0, 5.0);
  // print out the value you read:
        Serial.print(" Em volts: ");
  Serial.print(convertido);
  convertido = float(map(sensorValue, 0, 1023, -50.0, 150.0));
  // print out the value you read:
        Serial.print(" Depois de converter: ");
  Serial.println(convertido);
  delay(500); // delay in between reads for stability
}
